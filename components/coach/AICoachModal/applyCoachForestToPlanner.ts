...
import { coachTreesEqual } from "./diffCoachTree";
...
export function applyCoachForestToPlanner({
  planner,
  workingForest,
  userId,
  validCategoryIds,
  ...
  for (const goal of workingForest.goals) {
    ...
    if (isRetainedRoot) {
      ...
      if (canonicalGoal && coachTreesEqual(goal, canonicalGoal)) continue;
...
// Apply an accepted CoachNode tree onto an existing root. Preserves existing
...
function applyTreeToExistingRoot({
  ...
  // categories; null means "unspecified" (mergeCoachForest already backfills
...
function buildNewRootRows(
  ...
  function thread(child: CoachNode, parentId: string): void {
    ...
    rows.push({
      ...
      locationId: null,
      useParentLocation: false,
      // Every row in a new goal inherits the root's category; the coach
...
// which the coach contract doesn't carry. The prompt forbids it; this is the
// defensive coercion if the model does it anyway.
function normalizeRootType(
  raw: CoachNode["plannerType"],
  hasChildren: boolean,
): PlannerType {
  if (raw === "goal") return PlannerType.goal;
