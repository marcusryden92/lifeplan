...
// database: at Save, applyCoachForestToPlanner treats any id that matches no
// canonical row as new and mints the permanent UUIDs.
export function assignDraftIds(goals: CoachNode[]): {
  goals: CoachNode[];
...
function stampNode(node: CoachNode): CoachNode {
  return {
    ...node,
