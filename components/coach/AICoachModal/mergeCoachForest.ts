...
import type { CoachForestProposal } from "./normalizeCoachForest";
...
export function mergeCoachForest(
  base: CoachForest,
  proposal: CoachForestProposal,
): CoachForest {
  const baseById = new Map<string, CoachNode>();
  for (const goal of base.goals) {
    if (goal.id) baseById.set(goal.id, goal);
  }

  const replacedById = new Map<string, CoachNode>();
  const appended: CoachNode[] = [];
...
export function foldCoachProposals(
  base: CoachForest,
  orderedProposals: CoachForestProposal[],
): CoachForest {
  return orderedProposals.reduce(
    (forest, proposal) => mergeCoachForest(forest, proposal),
    base,
  );
}
