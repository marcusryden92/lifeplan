    ...
    <Shell active="Calendar">
      <div style={{ padding: '14px 22px', borderBottom: '1.5px dashed var(--pencil-light)', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[['week', true], ['day', false], ['list', false]].map(([v, sel]) => (
        ...
        <div className="sk-box wob-sm tight" style={{ background: 'var(--ink)', color: 'var(--paper)' }}>regenerate ↻</div>
      </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 340px', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {/* messages */}
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
              <span className="sk-script" style={{ fontSize: 24, fontWeight: 700 }}>Engine messages</span>
              <span className="sk-mono-tag">last gen · 2m ago</span>
            </div>
            <div className="sk-mono-tag" style={{ marginTop: 4 }}>3 warnings · 1 failure · proposed actions coming</div>

            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, overflow: 'auto', paddingRight: 4 }}>
              <Msg
                tone="fail"
                tag="FAIL"
          ...
          {/* legend */}
          <div style={{ padding: '12px 16px', borderTop: '1.5px dashed var(--pencil-light)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
              ...
              <Badge style={{ fontSize: 11 }}><Swatch color="#d6b9a2" /> Home</Badge>
              <Badge style={{ fontSize: 11 }}><Swatch color="#a2c8d6" /> Growth</Badge>
              <Badge kind="dim" style={{ fontSize: 11 }}>template</Badge>
              <Badge style={{ fontSize: 11, borderStyle: 'dashed' }}>travel</Badge>
            </div>
            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--pencil)' }}>
              <span className="sk-hatch-soft" style={{ display: 'inline-block', width: 24, height: 12, border: '1px solid var(--pencil)' }} />
...
function Msg({ tone, tag, title, body }) {
  const tones = {
    ...
    info: { border: 'var(--pencil-light)', bg: 'transparent', tagBg: 'var(--paper-2)', tagFg: 'var(--pencil)' }
  }[tone];
  return (
    <div className="sk-box wob-sm" style={{ padding: '8px 10px', background: tones.bg, borderColor: tones.border, borderWidth: tone === 'fail' ? 2 : 1.5 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{
          fontFamily: 'Special Elite, monospace', fontSize: 9, letterSpacing: 0.5,
          padding: '2px 6px', background: tones.tagBg, color: tones.tagFg,
          borderRadius: 3
        }}>{tag}</span>
        <span style={{ fontSize: 13.5, fontWeight: 700, lineHeight: 1.15, flex: 1 }}>{title}</span>
      </div>
      <div style={{ marginTop: 4, fontSize: 12.5, lineHeight: 1.35, color: 'var(--ink-soft)' }}>{body}</div>
      <div style={{ marginTop: 6, display: 'flex', gap: 6 }}>
        <span style={{ fontSize: 11, color: 'var(--pencil)' }}>actions:</span>
        <span style={{ fontSize: 11, color: 'var(--red-ink)' }}>(proposed soon)</span>
      </div>
    </div>
  );
}
...
function FullWeekCalendar() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* day header */}
      <div style={{ display: 'grid', gridTemplateColumns: '52px repeat(7, 1fr)', borderBottom: '2px solid var(--ink)', paddingBottom: 6 }}>
        <span />
        {days.map((d, i) => (
          <div key={d} style={{ textAlign: 'center', padding: '4px 0' }}>
            <div className="sk-mono-tag" style={{ fontSize: 10 }}>{d.split(' ')[0]}</div>
            <div className="sk-script" style={{ fontSize: 22, fontWeight: 700, lineHeight: 1, color: i === 2 ? 'var(--red-ink)' : 'var(--ink)' }}>{d.split(' ')[1]}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'auto' }}>
        <div style={{
          ...
          {hours.flatMap((h, ri) => [
            <span key={`hl${ri}`} className="sk-mono-tag" style={{ gridRow: ri+1, gridColumn: 1, paddingTop: 2, fontSize: 10 }}>{h}</span>,
          ...
          {[1, 3, 5].map(c => (
            <div key={`career${c}`} className="sk-hatch-soft" style={{
              gridRow: '3 / span 3', gridColumn: c+1, margin: 1
            }} />
          ))}
          {/* strict Health window Tue/Thu 6-8a (here partial) and Sat morning */}
          <div className="sk-hatch-soft" style={{ gridRow: '1 / span 2', gridColumn: 7, margin: 1, opacity: 0.5 }} />
          ...
          {/* tasks */}
          <Evt col={2} row="4 / span 3" tone="task" cat="career" label="Q4 doc · deep work" />
          <Evt col={4} row="3 / span 3" tone="task" cat="career" label="Q4 doc · deep work pt 2" emphasize />
          <Evt col={3} row="8 / span 1" tone="task" cat="home" label="plant basil ⚠" warn />
          <Evt col={3} row="9 / span 1" tone="task" cat="health" label="intervals 800×4" />
          <Evt col={5} row="11 / span 1" tone="task" cat="career" label="submit expenses" warn />

          {/* plans */}
          <Evt col={3} row="6 / span 1" tone="plan" label="1:1 ana" />
          <Evt col={6} row="10 / span 2" tone="plan" label="dentist · crown follow-up" />
          <Evt col={7} row="6 / span 2" tone="plan" label="brunch w/ T" />

          {/* travel */}
          <Evt col={3} row="7 / span 1" tone="travel" label="🚗 → home" partial />
          <Evt col={6} row="12 / span 1" tone="travel" label="🚗 → home" partial />
          ...
          <div style={{
            ...
            <span style={{
              position: 'absolute', left: `${(2/7) * 100}%`, top: -10,
              transform: 'translateX(-50%)',
              background: 'var(--red-ink)', color: 'var(--paper)',
              padding: '1px 6px', fontFamily: 'Special Elite, monospace', fontSize: 10, borderRadius: 3
            }}>NOW · 9:36a</span>
          </div>
        </div>
      </div>
...
function Evt({ col, row, tone, label, cat, emphasize, warn, partial }) {
  ...
  let style;
  if (tone === 'tmpl') {
    style = { background: 'var(--pencil-faint)', color: 'var(--ink-soft)', border: '1px dashed var(--pencil)' };
  } else if (tone === 'plan') {
    style = { background: 'var(--ink)', color: 'var(--paper)' };
  } else if (tone === 'travel') {
    style = { background: 'transparent', color: 'var(--pencil)', border: '1px dashed var(--pencil)', opacity: 0.8, fontSize: 10 };
  } else {
    style = {
