  ...
  return (
    <>
      ...
      <div style={{ padding: '30px 32px 22px', flexShrink: 0, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
        <div>
          <div style={{ fontFamily: CD, fontSize: 56, fontWeight: 500, letterSpacing: '-0.045em', lineHeight: 0.98, color: t.ink }}>
            Good morning, Marcus.
          </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button style={btnSolid(t)} onClick={() => onNav && onNav('calendar')}>Open calendar →</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 18, padding: '0 28px 24px', minHeight: 0 }}>
        ...
        <div style={{ ...g, borderRadius: 22, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${t.rule}`, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
            ...
            <button style={btnGlass(t)} onClick={() => onNav && onNav('calendar')}>Full week →</button>
          </div>
          <div style={{ flex: 1, overflow: 'auto', padding: '8px 12px 12px' }} className="noscroll">
            {ev.map((e, i) => {
              const ac = e.col ? area[e.col] : t.muted;
              return (
                <div key={i} onClick={() => onNav && onNav('item')} style={{
                  display: 'grid', gridTemplateColumns: '72px 1fr auto', gap: 14, alignItems: 'center',
                  padding: '10px 12px', margin: '4px 0', borderRadius: 14, cursor: 'pointer',
                  background: e.now ? (t.isDark ? `${t.coral}1a` : `${t.coral}1f`) : 'transparent',
                    ...
                    {!e.travel && (
                      <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{
                          ...
                          padding: '2px 9px', borderRadius: 999, background: ac, border: 'none',
                          color: '#fff', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase'
                        }}>{e.area}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, overflow: 'auto', minHeight: 0 }} className="noscroll">
          <div style={{ ...g, borderRadius: 22, padding: '16px 20px' }}>
            ...
            {goals.map((g2, i) => {
              ...
              return (
                <div key={i} onClick={() => onNav && onNav('item')} style={{ padding: '12px 0', borderTop: i ? `1px solid ${t.rule}` : 'none', cursor: 'pointer' }}>
                  ...
                  <div style={{ marginTop: 8, height: 6, borderRadius: 999, background: t.isDark ? 'rgba(255,255,255,0.08)' : 'rgba(22,20,42,0.08)', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', inset: 0, width: `${g2.pct}%`, background: `linear-gradient(90deg, ${ac}, ${ac}cc)`, borderRadius: 999 }} />
                  </div>
          ...
          <div style={{ ...g, borderRadius: 16, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: t.shadowSm }}>
            ...
            <button style={btnSolid(t, true)} onClick={() => onNav && onNav('calendar')}>Use it →</button>
          </div>
        </div>
      </div>
    </>
...
function LumenWeek({ t, area, onNav }) {
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '56px repeat(7, 1fr)', borderBottom: `1px solid ${t.rule}`, background: t.glassBgSoft }}>
        <div />
        {WEEK.days.map(d => (
          <div key={d.n} style={{ padding: '12px 0', textAlign: 'center', borderLeft: `1px solid ${t.rule}` }}>
            <LCaption t={t} style={{ fontSize: 10 }}>{d.d}</LCaption>
            <div style={{ fontFamily: CD, fontSize: 22, fontWeight: 500, marginTop: 3, color: d.today ? t.coral : t.ink, letterSpacing: '-0.03em', fontVariantNumeric: 'tabular-nums' }}>{d.n}</div>
          </div>
        ))}
      </div>
      <div style={{ flex: 1, overflow: 'auto', position: 'relative' }} className="noscroll">
        <div style={{ display: 'grid', gridTemplateColumns: '56px repeat(7, 1fr)', gridTemplateRows: `repeat(${hours.length}, ${hourHeight}px)`, position: 'relative' }}>
          {hours.map((h, ri) => (
            <React.Fragment key={h}>
              <div style={{ gridRow: ri+1, gridColumn: 1, padding: '3px 8px', fontSize: 10.5, color: t.muted, fontFamily: HS, fontWeight: 600, fontVariantNumeric: 'tabular-nums', borderTop: ri ? `1px solid ${t.rule}` : 'none' }}>{h}:00</div>
              {WEEK.days.map((_, ci) => (
                <div key={ci} style={{ gridRow: ri+1, gridColumn: ci+2, borderTop: ri ? `1px solid ${t.rule}` : 'none', borderLeft: `1px solid ${t.rule}` }} />
              ))}
            </React.Fragment>
          ))}
          ...
          {WEEK.strict.map((s, i) => {
            ...
            return (
              <div key={i} style={{ position: 'absolute', left: `calc(56px + (100% - 56px) * ${s.day} / 7)`, width: `calc((100% - 56px) / 7)`, top, height, background: t.isDark ? 'rgba(255,255,255,0.025)' : 'rgba(22,20,42,0.025)', pointerEvents: 'none' }} />
            );
          })}
          ...
          {WEEK.events.map((e, i) => {
            ...
            return (
              <div key={i} onClick={() => !e.travel && !isTmpl && onNav && onNav('item')} style={{
                position: 'absolute',
                ...
                borderRadius: 8, padding: '4px 8px', fontSize: 11.5, lineHeight: 1.2, overflow: 'hidden',
                fontFamily: HS, fontWeight: 600, opacity: isTmpl ? 0.75 : 1,
          ...
          <div style={{ position: 'absolute', left: `calc(56px + (100% - 56px) * 2 / 7)`, width: `calc((100% - 56px) / 7)`, top: (9.6 - 7) * hourHeight, height: 1.5, background: t.coral, zIndex: 5 }}>
...
function LumenGoal({ t, onNav }) {
  ...
  return (
    <>
      <LMast t={t}>
        <button onClick={() => onNav && onNav('item')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}><LCaption t={t}>Items</LCaption></button>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 32px 32px' }} className="noscroll">
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 22, borderBottom: `1px solid ${t.rule}` }}>
          {['Overview','Schedule','Subtasks','Activity'].map((tab, i) => (
            <div key={tab} style={{ padding: '10px 16px', fontSize: 13, fontWeight: 600, fontFamily: HS, color: i === 0 ? t.ink : t.inkSoft, borderBottom: i === 0 ? `2px solid ${t.coral}` : '2px solid transparent', marginBottom: -1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 18, marginTop: 22 }}>
            ...
            <div style={{ ...g, borderRadius: 20 }}>
              ...
              <div style={{ padding: '6px 10px 12px' }}>
                {goal.subtasks.slice(0, 6).map((s, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '20px 1fr auto auto', gap: 14, alignItems: 'center', padding: '8px 10px', borderRadius: 10, background: s.current ? `${t.coral}1a` : 'transparent', border: s.current ? `1px solid ${t.coral}55` : '1px solid transparent' }}>
                    <div style={{ width: 16, height: 16, borderRadius: 4, border: `1.5px solid ${s.done ? t.mint : (t.isDark ? 'rgba(230,232,236,0.30)' : 'rgba(22,20,42,0.30)')}`, background: s.done ? t.mint : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#fff', fontWeight: 700 }}>{s.done && '✓'}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ ...g, borderRadius: 20, padding: '18px 20px', background: `${ac}22`, border: `1px solid ${ac}55` }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <button style={{ ...btnGlass(t), padding: '7px 12px', fontSize: 11.5 }} onClick={() => onNav && onNav('calendar')}>View calendar</button>
  ...
  return (
    <>
      ...
      <div style={{ flex: 1, display: 'grid', placeItems: 'center', padding: 32 }}>
        <div style={{ ...g, borderRadius: 24, padding: '40px 44px', maxWidth: 460, textAlign: 'center' }}>
          <ConicDot t={t} size={18} />
          <div style={{ fontFamily: CD, fontSize: 30, fontWeight: 500, letterSpacing: '-0.03em', color: t.ink, marginTop: 16 }}>{label}</div>
          <div style={{ fontSize: 13.5, color: t.inkSoft, marginTop: 10, lineHeight: 1.55, fontFamily: HS, fontWeight: 500 }}>
            This surface still lives as a low-fi wireframe. It’ll be rebuilt onto the Lumen
            foundation in a later pass — Today, Calendar and Item detail are converted first.
          </div>
        </div>
      </div>
    </>
