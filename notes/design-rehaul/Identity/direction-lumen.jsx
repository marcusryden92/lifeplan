  ...
  return (
    <div className="ab" style={{
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <LumenPlate t={t} g={g} />
        <LumenWordmark t={t} g={g} />

        <LumenType t={t} g={g} />

        <LumenAppChrome t={t} g={g} />
        <LumenMarketing t={t} g={g} />
        <LumenVoice t={t} g={g} />
      </div>
    </div>
  );
    ...
    <div style={{ ...g, borderRadius: 20, padding: '20px 26px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <div>
        <div style={{
          ...
          <TriDot t={t} />
          {t.isDark ? '06 · Lumen Noir' : '05 · Lumen'}
        </div>
        <div style={{ fontFamily: '"Clash Display", sans-serif', fontSize: 46, lineHeight: 0.95, color: t.ink, fontWeight: 500, letterSpacing: '-0.04em' }}>
          {t.isDark ? 'Lumen / Noir' : 'Lumen'}
        </div>
        <div style={{ marginTop: 12, color: t.inkSoft, fontSize: 14, maxWidth: 580, lineHeight: 1.55, fontWeight: 500 }}>
          {t.isDark
            ? 'Same restraint, lights off. Glass over deep plum; three colors carry the work.'
            : 'Frosted glass over a clean paper canvas. Three colors do the heavy lifting — Coral for signal, Lavender for primary, Sky for context.'}
        </div>
      </div>
    ...
    <div style={{ marginTop: 28, ...g, borderRadius: 24, padding: '24px 28px' }}>
      ...
      <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 36 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            <LumenCaption t={t}>UI · Hubot Sans 500</LumenCaption>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <LumenCaption t={t}>Numerics · Hubot Sans tnum</LumenCaption>
          ...
          <div>
            <LumenCaption t={t}>Voice</LumenCaption>
...
function LumenAppChrome({ t, g }) {
  return (
    <div style={{ marginTop: 28 }}>
      <div style={{ marginBottom: 12 }}>
        <LumenSectionLabel t={t} num="ii" title="In the product" inline />
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '70px 1fr 1fr 1fr 1fr', minHeight: 260, position: 'relative' }}>
          ...
          {['Mon 3', 'Tue 4', 'Wed 5', 'Thu 6'].map((d, i) => (
            <div key={d} style={{ borderRight: i < 3 ? `1px solid ${t.rule}` : 'none', position: 'relative', padding: '10px 8px 0' }}>
              ...
              {i === 0 && <LumenEvent t={t} top={4} h={60} cat="Career" title="Quarterly review" time="09:00 — 10:15" color={t.coral} />}
              {i === 1 && (
                <>
                  <LumenEvent t={t} top={4} h={40} cat="Health" title="Morning swim" time="08:30 — 09:15" color={t.sky} />
                  <LumenEvent t={t} top={52} h={56} cat="Career" title="Deep work" time="10:00 — 11:30" color={t.lavender} />
                </>
              )}
              {i === 2 && <LumenEvent t={t} top={20} h={96} cat="Creative" title="Brand explorations" time="09:30 — 12:00" color={t.coral} />}
              {i === 3 && (
                <>
                  <LumenEvent t={t} top={4} h={40} cat="Social" title="Coffee · K" time="09:00 — 09:45" color={t.sky} />
                  <LumenEvent t={t} top={52} h={60} cat="Career" title="Pitch prep" time="10:00 — 11:30" color={t.lavender} />
                </>
              )}
            </div>
          ))}
...
function LumenEvent({ t, top, h, cat, title, time, color }) {
  return (
    <div style={{
...
function LumenMarketing({ t, g }) {
  return (
    <div style={{ marginTop: 28 }}>
      <div style={{ marginBottom: 12 }}>
        <LumenSectionLabel t={t} num="iii" title="On the web" inline />
      </div>
      <div style={{
...
function LumenVoice({ t, g }) {
  return (
    <div style={{ marginTop: 28, ...g, borderRadius: 20, padding: '20px 24px',
      ...
      <div>
        <LumenCaption t={t}>Not</LumenCaption>
...
function LumenSectionLabel({ t, num, title, inline }) {
  return (
    <div style={{
...
function LumenCaption({ t, children }) {
  return (
