...
`placeLeaf` calls this at leaf entry and takes the min with the leaf's allowed-times ceiling (`maxAllowedBlockMinutes` — the exact longest contiguous block a generic week offers under the constraint chain, measured on a two-week unroll so blocks chaining across midnight and the week seam count; the per-task value stays outside the category-keyed cache). When the leaf is ALSO window-constrained, `maxConstrainedBlockMinutes` takes the true weekly intersection of the allowed-times pattern and the eligible category windows — the min of the two independent ceilings cannot see disjoint patterns (windows Monday, allowed times Tuesday) — and a structurally empty intersection permanently fails the leaf as `IMPOSSIBLE_CONSTRAINTS` (a `TASK_UNSCHEDULABLE` message) while a small one tightens `maxCapacity`; the intersection check is skipped when an eligible window carries per-occurrence exceptions, since a moved occurrence can land inside the allowed times. If the required block exceeds `maxCapacity`, the task is marked `TOO_LARGE` immediately — no iterations wasted attempting placement, and no expansion budget burned hunting for an allowed block that cannot exist. The required block is the full duration for a plain task, but only the required **minimum chunk** for a split task (`minChunkRequired` of the run's remainder) — a split task is `TOO_LARGE` only when not even one minimum chunk fits anywhere.
